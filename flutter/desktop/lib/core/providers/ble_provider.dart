...
class BleUploadState with ChangeNotifier {
  ...
  Future<bool> scan({Duration timeout = const Duration(seconds: 6)}) async {
    ...
    try {
      ...
      if (adapterState != BluetoothAdapterState.on) {
        debugPrint('Bluetooth not ready, waiting...');
      ...
      debugPrint('✓ Bluetooth ready, starting scan...');
  ...
  Future<String?> upload() async {
    ...
    try {
      ...
      if (info.mtu > 0 && info.mtu <= _maxChunk) {
        ...
        debugPrint('⚠ Invalid MTU size ${info.mtu}, using default $_defaultChunk');
        chunk = _defaultChunk;
      }

      // Additional safety: ensure we never exceed platform limits
      if (chunk > _maxChunk) {
        debugPrint('⚠ Limiting chunk from $chunk to $_maxChunk for platform compatibility');
        chunk = _maxChunk;
      }

      // Log upload configuration
      debugPrint('📤 Upload Configuration:');
      debugPrint('   Size: ${data.length} bytes');
      debugPrint('   Dimensions: ${info.width}x${info.height}');
      debugPrint('   Display: ${info.displayType == 1 ? '6-color' : 'B/W'}');
      debugPrint('   Rotation: $rotation');
      debugPrint('   Chunk size: $chunk bytes (from device MTU: ${info.mtu})');
  ...
  Future<void> _discoverAndReadInfo() async {
    ...
    debugPrint('   Display: $displayName (${width}x$height)');
    debugPrint('   Rotation: $rotationValue');
