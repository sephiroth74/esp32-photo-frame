...
echo "Converting .svg to .png at ${1}x${1} resolution..."

if [ ! -e "$PNG_PATH" ]; then
  echo "Creating PNG directory at $PNG_PATH..."
  mkdir -p $PNG_PATH

  // use zsh for globbing to find all .svg files
  echo "Converting SVG files to PNG format..."
  # check if inkscape is installed
  if ! command -v inkscape &> /dev/null; then
    echo "Inkscape is not installed. Please install it to convert SVG files."
    exit 1
  fi

  # check if ImageMagick is installed
  if ! command -v mogrify &> /dev/null; then
    echo "ImageMagick is not installed. Please install it to convert SVG files."
    exit 1
  fi
