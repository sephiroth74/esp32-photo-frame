...
struct BatteryMappingStep {
   ...
   */
  constexpr BatteryMappingStep(uint16_t percent, uint16_t voltage) : percent(percent), voltage(voltage) {}
};
 ...
 * and charging status. The structure adapts based on whether the MAX1704X
 * sensor is being used or analog voltage reading is employed.
 */
struct BatteryInfo {
public:
  ...
  constexpr BatteryInfo(float cellVoltage, float chargeRate, float percent)
      : cellVoltage(cellVoltage), chargeRate(chargeRate), percent(percent), millivolts(static_cast<uint32_t>(cellVoltage * 1000)) {}
   ...
   */
  constexpr BatteryInfo(const BatteryMappingStep &step) : BatteryInfo(step.voltage / 1000.0f, 0.0f, step.percent) {}
  ...
  constexpr bool operator==(const BatteryInfo &other) const {
    return (cellVoltage == other.cellVoltage && chargeRate == other.chargeRate && percent == other.percent);
  }
   ...
   * @param percent Battery percentage (0-100)
   */
  constexpr BatteryInfo(uint32_t rawValue, uint32_t raw_millivolts, uint32_t millivolts, float percent)
      : raw_value(raw_value), raw_millivolts(raw_millivolts), millivolts(millivolts), percent(percent) {}
  ...
  constexpr BatteryInfo(const BatteryMappingStep &step)
      : raw_value(step.voltage), raw_millivolts(step.voltage), millivolts(step.voltage), percent(step.percent) {}
   ...
   */
  void updateFromAnalog(uint32_t rawValue, uint32_t rawMillivolts, uint32_t millivolts, float percent) {
