...
/**
 * @brief High-level display management class that encapsulates all rendering
 * components
 *
 ...
 *     loadImageToBuffer(display.getBuffer(), imageFile, filename, DISP_WIDTH,
 * DISP_HEIGHT); imageFile.close(); sdCard.end();  // Close SD card before
 * display operations
 *
...
public:
   ...
   * @param preferPsram Try to allocate buffer in PSRAM if available (default:
   * true)
  ...
  /**
   * @brief Check if display manager is fully initialized (both buffer and
   * hardware)
   ...
   * @param rotation 0=landscape, 1=portrait CCW, 2=landscape flipped,
   * 3=portrait CW
   */
   ...
   * @param gravity Position on screen (TOP_LEFT, TOP_RIGHT, BOTTOM_LEFT,
   * BOTTOM_RIGHT, TOP_CENTER)
   * @param message Message text to display
   * @param xOffset Optional X offset
   * @param yOffset Optional Y offset
   */
  void drawSideMessage(gravity_t gravity, const char *message, int32_t xOffset = 0, int32_t yOffset = 0);
   ...
   * @param xOffset Optional X offset
   */
  void drawSideMessageError(gravity_t gravity, photo_frame::photo_frame_error_t error, int32_t xOffset = 0, int32_t yOffset = 0);

  /**
   * @brief Set the image source for status reporting
   * @param source The source of the current image (CLOUD, LOCAL_CACHE,
   * BLUETOOTH)
   */
   ...
   */
  void drawErrorWithDetails(const String &errMsgLn1, const String &errMsgLn2, const char *filename, uint16_t errorCode);
