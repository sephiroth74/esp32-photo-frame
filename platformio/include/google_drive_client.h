...
// Google Drive page token buffer size (Google Drive tokens are typically
// 100-200 chars) "nextPageToken":
...
typedef struct {
  ...
  int backoffBaseDelayMs;     ///< Base delay for exponential backoff in
                              ///< milliseconds
 ...
 *
 * This class provides methods to authenticate with Google Drive via JWT, list
 * files in folders, download files to LittleFS, and manage access tokens. It
 ...
 *
 * @note Requires mbedtls for cryptographic operations and assumes LittleFS is
...
public:
  ...
  /**
   * @brief Lists files in a specified Google Drive folder and streams them
   * directly to a TOC file.
   *
   * This memory-efficient version writes files directly to the TOC file as they
   * are parsed from the API response, avoiding the need to keep all files in
   * memory at once.
   *
   ...
   * @param pageSize The maximum number of files to retrieve per request
   * (default is 50).
   * @return Total number of files written to the TOC, or 0 on error
   */
  size_t list_files_streaming(const char *folderId, SdCard &sdCard, const char *tocFilePath, int pageSize = 50);
   ...
   * @param marginSeconds Safety margin in seconds before actual expiration
   * (default: 60)
...
private:
  ...
  /**
   * @brief Lists files in a folder with pagination support, streaming directly
   * to TOC file.
   ...
   */
  size_t list_files_in_folder_streaming(const char *folderId, SdCard &sdCard, const char *tocFilePath, int pageSize = 10,
                                        char *nextPageToken = nullptr, const char *pageToken = "");

  /**
   * @brief Streaming parser that writes files directly to TOC file with
   * extension filtering.
   *
   * Parses Google Drive API JSON response and writes file entries to TOC file.
   * Automatically filters files based on allowed extensions at runtime,
   * supporting both binary (.pfr1) and bitmap (.bmp) formats.
   *
   ...
   *
   * @note Only files with extensions in ALLOWED_FILE_EXTENSIONS array are
   * written to TOC
   * @see ALLOWED_FILE_EXTENSIONS for supported file types
   */
  size_t parse_file_list_to_toc(const String &jsonBody, SdCard &sdCard, const char *tocFilePath, char *nextPageToken);
   ...
   */
  String build_http_request(const char *method, const char *path, const char *host, const char *headers = nullptr,
