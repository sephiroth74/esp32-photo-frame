...
/**
 * @brief Allocate memory preferring PSRAM if available, otherwise fallback to internal RAM
 *
...
/**
 * @brief Allocate memory preferring internal RAM if available, otherwise fallback to PSRAM
 *
 * @param size Size in bytes to allocate
 * @return void* Pointer to allocated memory, or nullptr if allocation failed
 */
...
/**
 * @brief Allocate and zero-initialize memory preferring internal RAM if available
 *
 * @param num Number of elements
 * @param size Size of each element in bytes
 * @return void* Pointer to allocated memory, or nullptr if allocation failed
...
/**
 * @brief Free memory allocated by heap_malloc/calloc/realloc or psram_malloc/calloc/realloc
 *
...
/**
 * @brief Free memory allocated by psram_malloc/calloc/realloc (alias for heap_free)
 *
 * @param ptr Pointer to memory to free (can be nullptr)
 */
 ...
 */
inline size_t psram_get_largest_free_block() {
...
inline void psram_print_info() {
    ...
    if (total > 0) {
        log_i("PSRAM: Total=%zu bytes, Free=%zu bytes (%.1f%%), Largest block=%zu bytes\n",
              total,
              free_size,
              (free_size * 100.0) / total,
              largest_block);
    } else {
 ...
 *
 * @param size Size in bytes to allocate
 * @return PSRAMUniquePtr Smart pointer to allocated memory, or nullptr if allocation failed
 *
 * Usage:
 *   auto buffer = make_psram_unique(1024); // Allocate 1KB
 *   buffer[0] = 42; // Use like array
 */
inline PSRAMUniquePtr make_psram_unique(size_t size) {
    return PSRAMUniquePtr(static_cast<uint8_t*>(psram_malloc(size)));
}
