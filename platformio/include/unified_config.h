...
struct unified_config {
    ...
    struct board_config {
        ...
        bool is_valid() const
        {
            return day_start_hour < 24 && day_end_hour < 24 && refresh.min_seconds > 0 && refresh.max_seconds > refresh.min_seconds && refresh.default_seconds >= refresh.min_seconds && refresh.default_seconds <= refresh.max_seconds && display_rotation < 4;
        }
    } board;
