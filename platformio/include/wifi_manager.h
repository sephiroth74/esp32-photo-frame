...
public:
   ...
   * The manager will try to connect to each network in order until one
   * succeeds.
   *
   * FAILOVER BEHAVIOR:
   ...
   *
   * @param wifi_config Reference to wifi_config structure containing networks
   * array
   * @return photo_frame_error_t Error status (None on success)
   ...
   * @return DateTime object containing current date and time, or invalid
   * DateTime on error
   ...
   *
   * @return String containing the network SSID, or empty string if not
   * connected
   * @note Returns the SSID from stored credentials, not necessarily the
   * connected network
   */
