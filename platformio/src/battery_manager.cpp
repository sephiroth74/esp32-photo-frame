...
photo_frame_error_t BatteryManager::readFromSensor(BatteryInfo& info) const {
    ...
    log_d("[BatteryManager] Battery reading: voltage: %.2fV, percent: %.1f%%, charge rate: %.2f mA",
          voltage,
          percent,
          charge_rate);
...
BatteryInfo BatteryManager::readFromAnalog() const {
    ...
    log_d("[BatteryManager] Battery reading: raw: %lu, millivolts: %lu, voltage: %lu, percent: %u",
          raw,
          millivolts,
          voltage,
          percent);
