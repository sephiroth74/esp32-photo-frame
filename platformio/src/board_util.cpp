...
void initDisplayPower() {
  ...
  delay(100);
#else
  log_d("[POWER] Display power control not configured (DISPLAY_POWER_PIN not "
        "defined)");
#endif
}
...
void enterDeepSleep(esp_sleep_wakeup_cause_t wakeup_reason, uint64_t refresh_microseconds) {
  ...
  if (wakeup_result == ESP_OK) {
    log_v("Testing RTC IO capability for GPIO %d... Pin state: %d | EXT1 "
          "config: SUCCESS",
          WAKEUP_PIN, pinState);
    log_v("GPIO %d configured for EXT1 wakeup with mask 0x%X, level: %s", WAKEUP_PIN, (uint32_t)pin_mask,
  ...
  // This is essential - regular pinMode pull resistors don't work during deep
  // sleep
  ...
  if (pinState != 1) {
    log_w("Testing RTC IO capability for GPIO %d... Pin state: %d - WARNING: "
          "Pin should be "
          "HIGH when idle - check wiring or RTC GPIO config",
          WAKEUP_PIN, pinState);
  } else {
    log_v("Testing RTC IO capability for GPIO %d... Pin state: %d - Pin "
          "correctly reads HIGH "
          "with RTC pull-up",
          WAKEUP_PIN, pinState);
  }
  ...
  if (finalPinState == WAKEUP_LEVEL) {
    log_w("Final wakeup pin state before sleep: %d - WARNING: Wakeup pin is "
          "already at trigger "
          "level!",
          finalPinState);
  ...
  if (refresh_microseconds > MICROSECONDS_IN_SECOND) {
    log_d("Configuring timer wakeup for %lu seconds", (unsigned long)(refresh_microseconds / 1000000ULL));
...
long readRefreshSeconds(const unified_config &config, photo_frame::BatteryInfo &BatteryInfo) {
  ...
  float exponential_position = linear_position * linear_position * linear_position; // Cube the position for more
  ...
  refresh_seconds =
      REFRESH_MIN_INTERVAL_SECONDS + (long)(exponential_position * (REFRESH_MAX_INTERVAL_SECONDS - REFRESH_MIN_INTERVAL_SECONDS));
...
bool checkHeapHealth(const char *context, size_t threshold) {
  ...
  if (free_heap < threshold) {
    log_w("Low heap memory in context '%s': %u bytes free (threshold: %u bytes)", context, free_heap, threshold);
    return false;
  } else {
