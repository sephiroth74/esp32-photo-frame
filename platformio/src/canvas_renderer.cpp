...
namespace canvas_renderer {
    ...
    static void draw_inverted_bitmap(GFXcanvas8& canvas, int16_t x, int16_t y,
    ...
    void draw_last_update(GFXcanvas8& canvas, const DateTime& lastUpdate, long refresh_seconds)
    {
        ...
        if (!portrait_mode) {
            draw_side_message_with_icon(canvas, gravity::TOP_LEFT, icon_name::wi_time_10, lastUpdateBuffer, 0, -2);
        } else {
        ...
        image_source_t image_source)
    {
        ...
        draw_side_message_with_icon(canvas, gravity::TOP_CENTER,
    ...
    void draw_error(GFXcanvas8& canvas, photo_frame_error_t error)
    {
        ...
        draw_string(canvas, canvas.width() / 2, canvas.height() / 2, errorMsg, CENTER, DISPLAY_COLOR_BLACK);
    }
        ...
        uint16_t errorCode)
    {
        ...
        draw_string(canvas, canvas.width() / 2, canvas.height() / 2 - 20, errMsgLn1.c_str(), CENTER, DISPLAY_COLOR_BLACK);

        // Draw secondary message if provided
        if (errMsgLn2.length() > 0) {
            draw_string(canvas, canvas.width() / 2, canvas.height() / 2, errMsgLn2.c_str(), CENTER, DISPLAY_COLOR_BLACK);
        }

        // Draw filename and error code if provided
        if (filename && errorCode > 0) {
            String details = String(filename) + " (Error: " + String(errorCode) + ")";
            draw_string(canvas, canvas.width() / 2, canvas.height() / 2 + 20, details.c_str(), CENTER, DISPLAY_COLOR_BLACK);
        }
    }
    ...
    void draw_rounded_rect(GFXcanvas8& canvas, int16_t x, int16_t y,
        ...
        uint16_t color, uint8_t transparency)
    {
