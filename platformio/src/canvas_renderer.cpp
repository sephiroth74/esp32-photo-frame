...
static void drawInvertedBitmap(GFXcanvas8 &canvas, int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h,
                               uint16_t color) {
...
void drawLastUpdate(GFXcanvas8 &canvas, const DateTime &lastUpdate, long refresh_seconds) {
  ...
  if (!portrait_mode) {
    drawSideMessageWithIcon(canvas, gravity::TOP_LEFT, icon_name::wi_time_10, lastUpdateBuffer, 0, -2);
  } else {
...
void drawImageInfo(GFXcanvas8 &canvas, uint32_t index, uint32_t total_images, ImageSource image_source) {
  String message = String(index + 1) + " / " + String(total_images);
  drawSideMessageWithIcon(canvas, gravity::TOP_CENTER,
                          image_source == photo_frame::ImageSource::IMAGE_SOURCE_CLOUD
                              ? icon_name::cloud_0deg
                              : (image_source == photo_frame::ImageSource::IMAGE_SOURCE_BLUETOOTH ? icon_name::bluetooth_0deg
...
void drawImageInfo(GFXcanvas8 &canvas, const String &message, ImageSource image_source) {
  ...
  case IMAGE_SOURCE_CLOUD:
    drawSideMessageWithIcon(canvas, gravity::TOP_CENTER, icon_name::cloud_0deg, message.c_str(), -4, 0);
    break;
  case IMAGE_SOURCE_BLUETOOTH:
    drawSideMessageWithIcon(canvas, gravity::TOP_CENTER, icon_name::bluetooth_0deg, message.c_str(), -4, 0);
    break;
  case IMAGE_SOURCE_WEBSOCKET:
    drawSideMessageWithIcon(canvas, gravity::TOP_CENTER, icon_name::wifi, message.c_str(), -4, 0);
    break;
  case IMAGE_SOURCE_LOCAL_CACHE:
    drawSideMessageWithIcon(canvas, gravity::TOP_CENTER, icon_name::micro_sd_card_0deg, message.c_str(), -4, 0);
    break;

  default:
    drawSideMessage(canvas, gravity::TOP_CENTER, message.c_str(), -4, 0);
    break;
  }
}
...
void drawCenteredMessageWithIcon(GFXcanvas8 &canvas, icon_name_t icon, const String &title, const String &message,
  ...
  const uint16_t max_text_width =
      disp_w > horizontal_padding ? static_cast<uint16_t>(disp_w - horizontal_padding) : static_cast<uint16_t>(disp_w);
  ...
  auto drawLines = [&](const std::vector<String> &lines, const GFXfont *font, int16_t line_gap) {
    ...
    for (size_t idx = 0; idx < lines.size(); ++idx) {
      ...
      drawString(canvas, center_x, baseline_y, lines[idx].c_str(), CENTER, DISPLAY_COLOR_BLACK);
...
void drawError(GFXcanvas8 &canvas, photo_frame_error_t error, const char *filename) {
  log_d("drawError: Starting with error code %d, filename: %s", error.code, filename ? filename : "N/A");
  ...
  log_d("drawError: About to draw multiline string - max_width: %d, "
        "total_padding: %d, "
        "max_lines: %d",
        text_max_width, multiline_error_h_padding, multiline_error_maxlines);

  // Draw multi-line error message
  drawMultilineString(canvas, disp_w / 2, baseY, errorMsg, CENTER,
                      text_max_width, // Use calculated max width
                      multiline_error_maxlines, 55, DISPLAY_COLOR_BLACK);
...
void drawErrorWithDetails(GFXcanvas8 &canvas, const String &errMsgLn1, const String &errMsgLn2, const char *filename,
  ...
  drawString(canvas, canvas.width() / 2, canvas.height() / 2 - 20, errMsgLn1.c_str(), CENTER, DISPLAY_COLOR_BLACK);

  // Draw secondary message if provided
  if (errMsgLn2.length() > 0) {
    drawString(canvas, canvas.width() / 2, canvas.height() / 2, errMsgLn2.c_str(), CENTER, DISPLAY_COLOR_BLACK);
  }

  // Draw filename and error code if provided
  if (filename && errorCode > 0) {
    String details = String(filename) + " (Error: " + String(errorCode) + ")";
    drawString(canvas, canvas.width() / 2, canvas.height() / 2 + 20, details.c_str(), CENTER, DISPLAY_COLOR_BLACK);
  }
}

void drawSideMessageWithIcon(GFXcanvas8 &canvas, gravity_t gravity, icon_name_t icon_name, const char *message, int32_t x_offset,
                             int32_t y_offset) {
  ...
  // Draw icon
  drawInvertedBitmap(canvas, icon_x, icon_y, icon, OVERLAY_ICON_SIZE, OVERLAY_ICON_SIZE, DISPLAY_COLOR_BLACK);
...
void drawSideMessage(GFXcanvas8 &canvas, gravity_t gravity, const char *message, int32_t x_offset, int32_t y_offset) {
  canvas.setFont(&FONT_7pt8b);
  canvas.setTextColor(DISPLAY_COLOR_BLACK);
...
void drawString(GFXcanvas8 &canvas, int16_t x, int16_t y, const char *text, alignment_t alignment, uint16_t color) {
...
void drawMultilineString(GFXcanvas8 &canvas, int16_t x, int16_t y, const String &text, alignment_t alignment, uint16_t max_width,
  ...
  while (current_line < max_lines && !textRemaining.isEmpty()) {
    ...
    while (w > max_width && splitAt != -1 && iterations < MAX_ITERATIONS) {
      ...
      // If splitAt == -1 then there is an unbroken set of characters longer
      // than max_width
      if (splitAt != -1) {
        endIndex = splitAt;
    ...
    for (int16_t dy = 0; dy < height; dy++) {
      for (int16_t dx = 0; dx < width; dx++) {
        ...
        if (draw_pixel) {
          ...
          if (dx < radius || dx >= width - radius || dy < radius || dy >= height - radius) {
