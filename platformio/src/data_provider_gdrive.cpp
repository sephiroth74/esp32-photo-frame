...
ImageLoadResult GoogleDriveDataProvider::load_next_image(bool is_reset, SdCard &sd_card, const unified_config &config) {
    ...
    if (error == photo_frame::error_type::None) {
      ...
      if (!shouldCleanup) {
        ...
        if (now - lastCleanup >= CLEANUP_TEMP_FILES_INTERVAL_SECONDS) {
          ...
          log_i("Skipping cleanup, only %ld seconds since last cleanup (need "
                "%d seconds)",
      ...
      if (error == photo_frame::error_type::None) {
        ...
        bool rebuild_toc = is_reset || !drive_.isMultiDirectoryTocValid(sd_card, config.GoogleDrive.drive.folder_ids);
        ...
        if (rebuild_toc) {
          ...
          if (!drive_.buildMultiDirectoryToc(sd_card, config.GoogleDrive.drive.folder_ids, false, &tocError)) {
            log_e("Failed to build Google Drive TOC cache: %s (code: %u)", tocError.message, tocError.code);
            sd_card.end();
      ...
      if (error == photo_frame::error_type::None) {
        ...
        for (const auto &folder_id : config.GoogleDrive.drive.folder_ids) {
          ...
          selectedFile = drive_.get_toc_file_by_name(sd_card, tocPath, GOOGLE_DRIVE_TEST_FILE, &tocError);
        ...
        if (!found_test) {
          log_w("Test file not found in TOC, falling back to random selection. "
                "Error: %d",
                tocError.code);
          if (!drive_.selectRandomImageFromFolders(sd_card, config.GoogleDrive.drive.folder_ids, selected_folder_id, selectedFile,
                                                   total_files, image_index, &tocError)) {
        ...
        if (tocError == photo_frame::error_type::None && selectedFile.id.length() > 0) {
          ...
          fs::File file;
          if (sd_card.fileExists(localFilePath.c_str()) && sd_card.getFileSize(localFilePath.c_str()) > 0) {
        ...
        log_i("Closing SD card after Google Drive error to prevent SPI "
              "conflicts");
        sd_card.end();
      }
    }
