...
DisplayManager::DisplayManager()
    : displayDriver_(nullptr) // unique_ptr starts as nullptr
      ,
      initialized_(false), rotation_(0), image_source_(IMAGE_SOURCE_NONE) {
...
bool DisplayManager::initBuffer(bool preferPsram) {
  ...
  log_d("[DisplayManager] Image buffer initialized (%ux%u = %u bytes, %s)", imageBuffer_.getWidth(), imageBuffer_.getHeight(),
...
std::unique_ptr<DisplayDriver> DisplayManager::createDisplayDriver() {
  ...
  return std::unique_ptr<DisplayDriver>(
      new DisplayDriver6C(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN));
#else
  log_i("[DisplayManager] Creating B/W display driver");
  return std::unique_ptr<DisplayDriver>(
      new DisplayDriverBW(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN));
#endif
}
...
void DisplayManager::drawSideMessage(gravity_t gravity, const char *message, int32_t xOffset, int32_t yOffset) {
...
void DisplayManager::drawSideMessageError(gravity_t gravity, photo_frame_error_t error, int32_t xOffset, int32_t yOffset) {
  ...
  photo_frame::drawSideMessageWithIcon(imageBuffer_.getCanvas(), gravity, icon_name::warning_icon, errorMsg, xOffset, yOffset);
}
...
void DisplayManager::drawImageInfo(uint32_t index, uint32_t total_images, ImageSource image_source) {
...
void DisplayManager::drawError(photo_frame_error_t error, const char *filename) {
  log_w("draw_error. code=%d, category%d, filename=%s", error.code, error.category, filename ? filename : "N/A");
  // Only need buffer to be initialized for drawing to canvas
  if (!imageBuffer_.isInitialized()) {
...
void DisplayManager::drawCenteredMessageWithIcon(GFXcanvas8 &canvas, icon_name_t icon, const String &title, const String &message,
                                                 uint16_t icon_size) {
...
void DisplayManager::drawErrorWithDetails(const String &errMsgLn1, const String &errMsgLn2, const char *filename,
                                          uint16_t errorCode) {
  log_d("[DisplayManager] Drawing error with details on canvas");
  // Only need buffer to be initialized for drawing to canvas
  if (!imageBuffer_.isInitialized())
    return;
  photo_frame::drawErrorWithDetails(imageBuffer_.getCanvas(), errMsgLn1, errMsgLn2, filename, errorCode);
}
...
void DisplayManager::refresh(bool partial_update) {
  log_d("[DisplayManager] Refreshing display (partial_update=%s)", partial_update ? "true" : "false");
  if (displayDriver_) {
