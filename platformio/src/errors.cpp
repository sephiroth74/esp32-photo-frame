...
photo_frame_error map_http_status_to_error(int statusCode, const char *context) {
  ...
  switch (statusCode) {
  case 400:
    error = error_type::HttpBadRequest;
    break;
  case 401:
    error = error_type::HttpUnauthorized;
    break;
  case 403:
    error = error_type::HttpForbidden;
    break;
  case 404:
    error = error_type::HttpNotFound;
    break;
  case 429:
    error = error_type::HttpTooManyRequests;
    break;
  case 500:
    error = error_type::HttpInternalServerError;
    break;
  case 502:
    error = error_type::HttpBadGateway;
    break;
  case 503:
    error = error_type::HttpServiceUnavailable;
    break;
  case 504:
    error = error_type::HttpGatewayTimeout;
    break;
  default:
...
photo_frame_error map_GoogleDrive_error(int statusCode, const char *responseBody, const char *context) {
  photo_frame_error error;
  ...
  if (responseBody) {
    ...
    case 404:
      ...
      break;
    case 429:
      error = error_type::GoogleDriveApiRateLimited;
      break;
    case 500:
    case 502:
    case 503:
      error = error_type::GoogleDriveApiInternalError;
      break;
    case 504:
      error = error_type::GoogleDriveNetworkTimeout;
      break;
    default:
      error = map_http_status_to_error(statusCode, "Google Drive API");
      break;
...
photo_frame_error create_image_error(const char *errorType, const char *filename, const char *dimensions, const char *context) {
  photo_frame_error error;
  ...
  } else if (strcmp(errorType, "size_mismatch") == 0) {
    error = error_type::ImageSizeInvalid;
  ...
  // Context removed to save DRAM - error message provides sufficient
  // information
...
photo_frame_error validate_image_dimensions(int width, int height, int maxWidth, int maxHeight, const char *filename) {
  char dimensions[64];
...
photo_frame_error validate_image_file_size(size_t fileSize, size_t expectedMinSize, size_t expectedMaxSize, const char *filename) {
  if (fileSize == 0) {
...
photo_frame_error create_battery_error(const char *errorType, float voltage, float percentage, float temperature,
                                       const char *context) {
  photo_frame_error error;
  ...
  // Context removed to save DRAM - error message provides sufficient
  // information - error message provides sufficient information

  error.set_timestamp();
  return error;
}

photo_frame_error validate_battery_voltage(float voltage, float minVoltage, float maxVoltage, const char *context) {
  ...
  if (voltage > maxVoltage) {
    char contextStr[128];
    snprintf(contextStr, sizeof(contextStr), "Voltage %.2fV exceeds maximum %.2fV%s%s", voltage, maxVoltage, context ? ", " : "",
             context ? context : "");
...
photo_frame_error validate_battery_temperature(float temperature, float minTemp, float maxTemp, const char *context) {
...
photo_frame_error create_charging_error(const char *errorType, float current, float voltage, const char *context) {
  photo_frame_error error;
