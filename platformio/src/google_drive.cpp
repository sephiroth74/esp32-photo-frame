...
size_t GoogleDrive::retrieve_toc(SdCard &sdCard, bool batteryConservationMode) {
  if (config.drive.folder_ids.empty()) {
    ...
    last_error = error_type::InvalidConfigNoImageSource;
    return 0;
  }

  const String &folder_id = config.drive.folder_ids[0];
  return retrieve_toc_for_folder(sdCard, folder_id, get_toc_file_path_for_folder(folder_id),
                                 get_toc_meta_file_path_for_folder(folder_id), batteryConservationMode);
}

size_t GoogleDrive::retrieve_toc_for_folder(SdCard &sdCard, const String &folder_id, const String &tocDataPath,
                                            const String &tocMetaPath, bool batteryConservationMode) {
  ...
  String tocFullPath = tocDataPath;
  bool shouldFetchFromDrive = false;
  bool localTocExists = sdCard.fileExists(tocFullPath.c_str()) && sdCard.getFileAge(tocDataPath.c_str()) >= 0;
  ...
  if (localTocExists) {
    ...
    if (fileTime > 0 && currentTime > 0) {
      ...
      log_i("TOC file age: %lu seconds, max age: %lu", fileAge, config.caching.toc_max_age_seconds);
      ...
      if (fileAge <= config.caching.toc_max_age_seconds) {
        ...
        log_i("TOC file is old but using cached version due to battery "
              "conservation mode");
        return get_toc_file_count(sdCard, tocFullPath);
      } else {
      ...
      log_i("Could not determine TOC file timestamp, but using existing file "
            "due to battery "
            "conservation mode");
      return get_toc_file_count(sdCard, tocFullPath);
    } else {
      log_w("Could not determine TOC file timestamp, will try to fetch from "
            "Google Drive");
      shouldFetchFromDrive = true;
    }
  } else if (!localTocExists) {
    if (batteryConservationMode) {
      log_i("TOC file does not exist locally and battery conservation mode is "
            "on - returning "
            "0");
  ...
  if (shouldFetchFromDrive) {
    ...
    if (error != error_type::None) {
      ...
      if (localTocExists) {
        ...
        if (count == 0 && toc_error != error_type::None) {
          // Local TOC also failed - report the original access token error
          // instead
    ...
    // Stream files directly from Google Drive to TOC file to save memory
    size_t totalFiles = client.list_files_streaming(folder_id.c_str(), sdCard, tocDataPath.c_str(), config.drive.list_page_size);
...
photo_frame_error_t GoogleDrive::create_directories(SdCard &sdCard) {
  ...
  // first make sure the directory doesn't exists and it's not a file, in which
  // case we will try to delete the file and continue otherwise, if the
  // directory already exists, continue to check subdirectories.
  if (sdCard.fileExists(GOOGLE_DRIVE_CACHING_LOCAL_PATH)) {
    if (sdCard.isFile(GOOGLE_DRIVE_CACHING_LOCAL_PATH)) {
      log_i("Removing file at path: %s", GOOGLE_DRIVE_CACHING_LOCAL_PATH);
      if (!sdCard.remove(GOOGLE_DRIVE_CACHING_LOCAL_PATH)) {
        log_e("Failed to remove file at path: %s", GOOGLE_DRIVE_CACHING_LOCAL_PATH);
        return photo_frame::error_type::SdCardDirCreateFailed;
      }
    } else {
      log_i("Root directory already exists: %s", GOOGLE_DRIVE_CACHING_LOCAL_PATH);
    }
  }

  // Create root directory
  if (!sdCard.createDirectories(GOOGLE_DRIVE_CACHING_LOCAL_PATH)) {
    log_e("Failed to create root directory for Google Drive local cache: %s", GOOGLE_DRIVE_CACHING_LOCAL_PATH);
    return photo_frame::error_type::SdCardDirCreateFailed;
  }
...
String GoogleDrive::get_toc_directory_path(const String &folder_id) const {
  String sanitized = folder_id;
...
String GoogleDrive::get_toc_meta_file_path_from_data_path(const String &tocDataPath) const {
  int lastSlash = tocDataPath.lastIndexOf('/');
  if (lastSlash >= 0) {
    return tocDataPath.substring(0, lastSlash + 1) + TOC_META_FILENAME;
  }
  return get_toc_meta_file_path();
}
...
String GoogleDrive::get_cache_dir_path() const {
  return GOOGLE_DRIVE_CACHING_LOCAL_PATH; // /.cache/gdrive
}
...
size_t GoogleDrive::get_toc_file_count(SdCard &sdCard, const String &filePath, photo_frame_error_t *error) {
  if (error) {
    *error = error_type::None;
  }

  // Build path to metadata file
  String tocMetaPath = get_toc_meta_file_path_from_data_path(filePath);
...
GoogleDriveFile GoogleDrive::get_toc_file_by_index(SdCard &sdCard, const String &filePath, size_t index,
...
bool GoogleDrive::buildMultiDirectoryToc(SdCard &sdCard, const std::vector<String> &folder_ids, bool batteryConservationMode,
                                         photo_frame_error_t *error) {
  if (folder_ids.empty()) {
    if (error) {
      *error = error_type::InvalidConfigNoImageSource;
    }
    return false;
  }

  if (!sdCard.createDirectories(GOOGLE_DRIVE_TOC_BASE_PATH)) {
    log_e("Failed to create TOC base directory: %s", GOOGLE_DRIVE_TOC_BASE_PATH);
    if (error) {
      *error = error_type::SdCardDirCreateFailed;
    }
    return false;
  }
  ...
  for (const auto &folder_id : folder_ids) {
    ...
    String tocDir = get_toc_directory_path(folder_id);
    if (!sdCard.createDirectories(tocDir.c_str())) {
      log_e("Failed to create TOC directory for folder: %s", folder_id.c_str());
      if (error) {
        *error = error_type::SdCardDirCreateFailed;
      }
      return false;
    ...
    size_t count = retrieve_toc_for_folder(sdCard, folder_id, get_toc_file_path_for_folder(folder_id),
                                           get_toc_meta_file_path_for_folder(folder_id), batteryConservationMode);
    if (count == 0 && last_error != error_type::None) {
      if (error) {
        *error = last_error;
      }
      return false;
...
bool GoogleDrive::isMultiDirectoryTocValid(SdCard &sdCard, const std::vector<String> &folder_ids) const {
  if (folder_ids.empty()) {
    return false;
  }

  if (!sdCard.isDirectory(GOOGLE_DRIVE_TOC_BASE_PATH)) {
    log_i("TOC base directory missing: %s", GOOGLE_DRIVE_TOC_BASE_PATH);
    return false;
  }

  std::vector<String> expectedPaths;
  expectedPaths.reserve(folder_ids.size());
  for (const auto &folder_id : folder_ids) {
    if (!folder_id.isEmpty()) {
      expectedPaths.push_back(get_toc_directory_path(folder_id));
    }
  }

  std::vector<String> cachedPaths;
  collect_toc_directories_with_files(sdCard, GOOGLE_DRIVE_TOC_BASE_PATH, cachedPaths);

  if (cachedPaths.size() != expectedPaths.size()) {
    log_i("TOC cache mismatch: expected %u directories, found %u", (unsigned int)expectedPaths.size(),
  ...
  for (const auto &folder_id : folder_ids) {
    ...
    String tocDataPath = get_toc_file_path_for_folder(folder_id);
    String tocMetaPath = get_toc_meta_file_path_for_folder(folder_id);
    if (!sdCard.fileExists(tocDataPath.c_str()) || !sdCard.fileExists(tocMetaPath.c_str())) {
      log_i("TOC invalid for folder: %s", folder_id.c_str());
      return false;
    }
  }
...
bool GoogleDrive::selectRandomImageFromFolders(SdCard &sdCard, const std::vector<String> &folder_ids, String &out_folder_id,
                                               GoogleDriveFile &out_file, uint32_t &out_total_files, uint32_t &out_selected_index,
                                               photo_frame_error_t *error) {
  out_folder_id = "";
  out_total_files = 0;
  out_selected_index = 0;
  out_file = GoogleDriveFile();
  ...
  std::vector<String> shuffled = folder_ids;
  for (int i = (int)shuffled.size() - 1; i > 0; --i) {
    int j = random(0, i + 1);
    std::swap(shuffled[i], shuffled[j]);
  }

  for (const auto &folder_id : shuffled) {
    if (folder_id.isEmpty()) {
      continue;
    }

    photo_frame_error_t countError = error_type::None;
    String tocDataPath = get_toc_file_path_for_folder(folder_id);
    size_t fileCount = get_toc_file_count(sdCard, tocDataPath, &countError);
    if (countError != error_type::None || fileCount == 0) {
      continue;
    }

    uint32_t selected_index = random(0, (long)fileCount);
    photo_frame_error_t fileError = error_type::None;
    GoogleDriveFile selected = get_toc_file_by_index(sdCard, tocDataPath, selected_index, &fileError);
    if (fileError != error_type::None || selected.id.length() == 0) {
      if (error) {
        *error = fileError;
      }
      continue;
    ...
    out_file = selected;
    out_total_files = (uint32_t)fileCount;
...
void GoogleDrive::collect_toc_directories_with_files(SdCard &sdCard, const char *base_path, std::vector<String> &out_paths) const {
  fs::File dir = sdCard.open(base_path, FILE_READ);
  ...
  while (!fileName.isEmpty()) {
    if (isDir) {
      String dataPath = fileName + "/" + TOC_DATA_FILENAME;
      String metaPath = fileName + "/" + TOC_META_FILENAME;
      if (sdCard.fileExists(dataPath.c_str()) && sdCard.fileExists(metaPath.c_str())) {
        out_paths.push_back(fileName);
      }
      collect_toc_directories_with_files(sdCard, fileName.c_str(), out_paths);
    }
...
fs::File GoogleDrive::download_file(SdCard &sdCard, GoogleDriveFile file, photo_frame_error_t *error) {
  fs::File emptyFile;
  ...
  // Verify final file size (no rename needed - downloaded directly to final
  // location)
...
uint32_t GoogleDrive::cleanup_temporary_files(SdCard &sdCard, boolean force) {
  log_i("Cleaning up temporary files... (Local path: %s, Force: %d)", GOOGLE_DRIVE_CACHING_LOCAL_PATH, force);
  ...
  log_i("SD card space - Total: %llu MB, Free: %llu MB (%llu%%), Threshold: "
        "%llu MB",
  ...
  String tempDir = GOOGLE_DRIVE_TEMP_DIR;
  ...
  if (force) {
    ...
    String accessTokenPath = string_utils::buildPath(GOOGLE_DRIVE_CACHING_LOCAL_PATH, ACCESS_TOKEN_FILENAME);
    ...
    // 2. Remove TOC files
    if (sdCard.isDirectory(GOOGLE_DRIVE_TOC_BASE_PATH)) {
      if (sdCard.cleanupDir(GOOGLE_DRIVE_TOC_BASE_PATH)) {
        cleanedCount++;
        log_i("Removed TOC directory: %s", GOOGLE_DRIVE_TOC_BASE_PATH);
      }
    } else {
    ...
    if (sdCard.isDirectory(GOOGLE_DRIVE_TOC_BASE_PATH)) {
      log_i("TOC integrity check skipped for multi-folder cache");
    } else {
      String tocDataPath = get_toc_file_path();
      String tocMetaPath = get_toc_meta_file_path();
...
uint32_t GoogleDrive::cleanup_all_cached_images(SdCard &sdCard) {
  uint32_t cleanedCount = 0;
  String cacheDir = GOOGLE_DRIVE_CACHING_LOCAL_PATH;
...
photo_frame_error_t GoogleDrive::save_access_token_to_file() {
  ...
  String tokenPath = string_utils::buildPath(GOOGLE_DRIVE_CACHING_LOCAL_PATH, ACCESS_TOKEN_FILENAME);
...
photo_frame_error_t GoogleDrive::initialize_from_unified_config(const unified_config::GoogleDrive_config &gd_config) {
  ...
  if (!gd_config.drive.use_insecure_tls && !gd_config.drive.root_ca_path.isEmpty()) {
    log_w("Warning: SSL certificate path specified but cannot load without SD "
          "card access");
    log_w("Consider using the JSON-based initialization for SSL certificate "
          "support");
  }
...
GoogleDriveFile GoogleDrive::get_toc_file_by_index(SdCard &sdCard, size_t index, photo_frame_error_t *error) {
...
GoogleDriveFile GoogleDrive::get_toc_file_by_name(SdCard &sdCard, const char *filename, photo_frame_error_t *error) {
  String tocPath = get_toc_file_path();
  return get_toc_file_by_name(sdCard, tocPath, filename, error);
}

GoogleDriveFile GoogleDrive::get_toc_file_by_name(SdCard &sdCard, const String &filePath, const char *filename,
                                                  photo_frame_error_t *error) {
  GoogleDriveTocParser parser(sdCard, filePath.c_str());
