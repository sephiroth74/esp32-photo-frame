...
static String base64url(const String &s) { return base64url((const uint8_t *)s.c_str(), s.length()); }
...
GoogleDriveClient::GoogleDriveClient(const GoogleDriveClient_config &config)
    : config(&config), g_access_token{.accessToken = {0}, .expiresAt = 0, .obtainedAt = 0}, lastRequestTime(0),
      requestHistoryIndex(0), requestCount(0) {
...
bool GoogleDriveClient::rsaSignRS256(const String &input, String &sig_b64url) {
  ...
  // ESP32-C6 and newer variants use updated mbedtls API with additional
  // parameters
  ...
  ret = mbedtls_pk_sign(&pk, MBEDTLS_MD_SHA256, hash, sizeof(hash), sig.get(), MBEDTLS_PK_SIGNATURE_MAX_SIZE, &sig_len,
  ...
  while (attempt < config->maxRetryAttempts) {
    ...
    if (config->useInsecureTls) {
      ...
      if (g_google_root_ca.length() > 0) {
        ...
        log_w("WARNING: No root CA certificate loaded, using insecure "
              "connection");
        client.setInsecure();
      }
    }
    ...
    if (failureType == failure_type::Permanent) {
      ...
      if (statusCode == 400) {
        ...
        if (responseBody.indexOf("invalid_grant") >= 0) {
          ...
          log_e("Error: invalid_client - Client ID or private key may be "
                "incorrect");
      ...
      return photo_frame::error_utils::map_GoogleDrive_error(statusCode, responseBody.c_str(), "OAuth token request");
    }
    ...
    if (statusCode == 200 && responseBody.length() > 0) {
      ...
      if (err) {
       ...
       // Check if this is an OAuth error response
        if (responseBody.indexOf("invalid_grant") >= 0 || responseBody.indexOf("invalid_client") >= 0) {
...
photo_frame_error_t GoogleDriveClient::download_file(const String &fileId, fs::File *outFile) {
  ...
  while (attempt < config->maxRetryAttempts) {
    ...
    log_i("Connection attempt took %lu ms (timeout: %d ms)", connectDuration, HTTP_CONNECT_TIMEOUT);
...
const GoogleDrive_access_token *GoogleDriveClient::get_access_token_value() const { return &g_access_token; }
...
bool GoogleDriveClient::parse_http_response(WiFiClientSecure &client, HttpResponse &response) {
  ...
  if (responseHeaders.isChunked) {
    ...
    // Handle regular content (with or without Content-Length) using efficient
    // block reading
    ...
    while ((client.connected() || client.available()) && bytesRemaining > 0) {
      ...
      // For responses without Content-Length, break if no more data is
...
bool GoogleDriveClient::handle_transient_failure(uint8_t attempt, failure_type failureType) {
  ...
  log_w("Transient failure (%s). Backing off for %lums (attempt %u/%u)", failureTypeName, backoffDelay, attempt + 1,
...
bool GoogleDriveClient::is_token_expired(int marginSeconds) { return g_access_token.expired(marginSeconds); }
...
String GoogleDriveClient::build_http_request(const char *method, const char *path, const char *host, const char *headers,
                                             const char *body) {
...
size_t GoogleDriveClient::list_files_streaming(const char *folderId, SdCard &sdCard, const char *tocFilePath, int pageSize) {
  ...
  do {
    ...
    size_t filesThisPage = list_files_in_folder_streaming(folderId, sdCard, tocFilePath, pageSize, nextPageToken, nextPageToken);
    if (filesThisPage == 0) {
...
#ifdef DEBUG_GOOGLE_DRIVE
    log_d("Page %d complete: got %zu files, total now %zu, nextPageToken "
          "size='%zu'",
          pageNumber, filesThisPage, totalFilesWritten, strlen(nextPageToken));
#endif // DEBUG_GOOGLE_DRIVE
...
size_t GoogleDriveClient::list_files_in_folder_streaming(const char *folderId, SdCard &sdCard, const char *tocFilePath,
                                                         int pageSize, char *nextPageToken, const char *pageToken) {
  log_i("list_files_in_folder_streaming folderId=%s, tocFilePath=%s, "
        "pageToken=%s",
        folderId, tocFilePath, pageToken);
...
size_t GoogleDriveClient::parse_file_list_to_toc(const String &jsonBody, SdCard &sdCard, const char *tocFilePath,
                                                 char *nextPageToken) {
