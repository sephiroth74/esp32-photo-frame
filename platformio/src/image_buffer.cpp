...
ImageBuffer::ImageBuffer() : buffer_(nullptr), canvas_(nullptr), bufferSize_(0), width_(0), height_(0), inPsram_(false) {}
...
bool ImageBuffer::init(uint16_t width, uint16_t height, bool preferPsram) {
  ...
  if (preferPsram) {
    ...
    if (buffer_) {
      ...
      log_i("Successfully allocated %u bytes from %s", bufferSize_, inPsram_ ? "PSRAM" : "internal RAM");
    }
  } else {
  ...
  canvas_ = new GFXcanvas8(width_, height_,
                           false); // false = don't allocate internal buffer
  if (!canvas_) {
...
void ImageBuffer::release() {
  ...
  if (buffer_) {
    log_d("Releasing image buffer at %p (%u bytes, %s)", buffer_, bufferSize_, inPsram_ ? "PSRAM" : "heap");
    photo_frame::psram_free(buffer_);
    buffer_ = nullptr;
    bufferSize_ = 0;
    width_ = 0;
...
ImageBuffer::ImageBuffer(ImageBuffer &&other) noexcept
    : buffer_(other.buffer_), canvas_(other.canvas_), bufferSize_(other.bufferSize_), width_(other.width_), height_(other.height_),
