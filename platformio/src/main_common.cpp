...
bool initializeHardware() {
  ...
  startupTime = millis();
  photo_frame::board_utils::blinkBuiltinLed(1, 900, 100);
  photo_frame::board_utils::disableBuiltinLed();
  ...
  if (psramFound()) {
    ...
    if (ret != ESP_OK) {
      ...
      // PSRAM should be automatically added to heap with
      // CONFIG_SPIRAM_USE_MALLOC which is set in platformio.ini via
  ...
  photo_frame::board_utils::printBoardStatistics();

#if DEBUG_MODE
  photo_frame::board_utils::printBoardPinConfiguration();
#endif // DEBUG_MODE
...
photo_frame::photo_frame_error_t setupBatteryAndPower(photo_frame::BatteryInfo &batteryInfo,
  ...
  if (batteryInfo.isEmpty()) {
    ...
    RGB_SET_STATE_TIMED(BATTERY_LOW,
                        3000); // Show battery critical warning briefly
...
refresh_delay_t calculateWakeupDelay(photo_frame::BatteryInfo &BatteryInfo, DateTime &now) {
  ...
  // if the battery level is low, use the battery low multiplier to reduce
  // refresh rate if the battery level is critical the device should just
  ...
  refresh_delay.refresh_seconds = photo_frame::board_utils::readRefreshSeconds(systemConfig, BatteryInfo);
  ...
  if (refresh_delay.refresh_seconds > 0) {
    ...
    if (refresh_delay.refresh_seconds <= 0) {
      log_w("Warning: Invalid refresh interval, using minimum interval as "
            "fallback");
  ...
  log_d("Final refresh delay: %ld seconds (%lu microseconds)", refresh_delay.refresh_seconds,
...
photo_frame::photo_frame_error_t renderImage(const photo_frame::PFR1BinaryFile &image_file, const char *original_filename,
  ...
  if (error == photo_frame::error_type::None) {
    ...
    if (rendering_failed) {
      ...
      display.drawErrorWithDetails(TXT_IMAGE_FORMAT_NOT_SUPPORTED, "", original_filename, error_code);
