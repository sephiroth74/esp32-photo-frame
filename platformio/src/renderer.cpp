...
// Static display driver instance - created at compile time based on
// configuration
#ifdef DISP_6C
static DisplayDriver6C displayDriver(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN);
#else
static DisplayDriverBW displayDriver(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN);
#endif
...
void rendererDrawMultilineString(GFXcanvas8 &canvas, int16_t x, int16_t y, const String &text, alignment_t alignment,
                                 uint16_t max_width, uint16_t max_lines, int16_t line_spacing, uint16_t color) {

  // Draw on canvas using canvas_renderer
  drawMultilineString(canvas, x, y, text, alignment, max_width, max_lines, line_spacing, color);
}

void drawSideMessageWithIcon(Adafruit_GFX &gfx, gravity_t gravity, icon_name_t icon_name, const char *message, int32_t x_offset,
                             int32_t y_offset) {
...
void drawSideMessage(Adafruit_GFX &gfx, gravity_t gravity, const char *message, int32_t x_offset, int32_t y_offset) {
...
void drawImageInfo(Adafruit_GFX &gfx, uint32_t index, uint32_t total_images, photo_frame::ImageSource image_source) {
  // This version draws directly on the provided GFX object
  // We assume it's a GFXcanvas8 - caller must ensure this
  GFXcanvas8 &canvas = static_cast<GFXcanvas8 &>(gfx);
...
uint16_t loadImageToBuffer(uint8_t *buffer, PFR1BinaryFile &file, const char *filename, int width, int height) {
