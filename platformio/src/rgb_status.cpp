...
// Predefined status configurations - optimized for power efficiency (max
// brightness 64)
const StatusConfig RGBStatus::STATUS_CONFIGS[] = {
    ...
    {SystemState::ERROR, RGBColors::RED, RGBEffect::BLINK_FAST, 0, 48}, // Keep error reasonably bright for visibility
...
RGBStatus::RGBStatus()
    : pixels(nullptr), rgbTaskHandle(nullptr), currentState(SystemState::IDLE), currentConfig(SystemState::IDLE, RGBColors::OFF),
      enabled(true), taskRunning(false), lastUpdate(0), effectStep(0), currentBrightness(0) {}
...
void RGBStatus::setCustomColor(const RGBColor &color, RGBEffect effect, uint16_t duration_ms, uint8_t brightness) {
  if (!enabled || !pixels)
    return;
