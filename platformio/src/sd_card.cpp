...
photo_frame_error_t SdCard::begin() {
  ...
  // Too fast (2MHz+) causes timeouts, too slow (400kHz) may have issues during
  // operations
  ...
  SD_CARD_LIB.setPins(SD_MMC_CLK_PIN, SD_MMC_CMD_PIN, SD_MMC_D0_PIN, SD_MMC_D1_PIN, SD_MMC_D2_PIN, SD_MMC_D3_PIN);
...
std::vector<String> SdCard::listFilesInDirectory(const char *dir_path, const char *extension) const {
...
String SdCard::getTocDataPath(const char *dir_path) const { return this->getTocDirectoryPath(dir_path) + "/" + TOC_DATA_FILENAME; }

String SdCard::getTocMetaPath(const char *dir_path) const { return this->getTocDirectoryPath(dir_path) + "/" + TOC_META_FILENAME; }

void SdCard::collectTocDirectoriesWithFiles(const char *base_path, std::vector<String> &out_paths) const {
...
bool SdCard::buildDirectoryToc(const char *dir_path, const char *extension, photo_frame_error_t *error) {
  if (!initialized) {
    log_e("SD card not initialized.");
    if (error)
...
bool SdCard::isTocValid(const char *dir_path, const char *extension) const {
  ...
  SdCardTocParser parser(const_cast<SdCard &>(*this), this->getTocDataPath(dir_path).c_str(),
  ...
  if (!parser.validate_toc(dir_path, extension)) {
    log_i("TOC rebuild reason: TOC validation failed (path or extension "
          "mismatch)");
    return false;
  }

  // If we reach here, the TOC files exist and are valid for the requested
  // directory Update the cached values so subsequent calls in the same session
  // are faster
...
uint32_t SdCard::countFilesCached(const char *dir_path, const char *extension, bool use_toc) const {
  if (use_toc && this->shouldUseToc(dir_path, extension)) {
    ...
    if (error.code == 0) { // No error
      ...
      log_w("TOC read failed: %s (code: %u), falling back to direct iteration", error.message, error.code);
    }
  }
...
String SdCard::getFileAtIndexCached(const char *dir_path, uint32_t index, const char *extension, bool use_toc) const {
  if (use_toc && this->shouldUseToc(dir_path, extension)) {
    SdCardTocParser parser(const_cast<SdCard &>(*this), this->getTocDataPath(dir_path).c_str(),
                           this->getTocMetaPath(dir_path).c_str());
...
bool SdCard::buildMultiDirectoryToc(const std::vector<String> &directories, const char *extension, photo_frame_error_t *error) {
  if (!initialized) {
    if (error)
      *error = error_type::NoSdCardAttached;
    return false;
...
bool SdCard::isMultiDirectoryTocValid(const std::vector<String> &directories, const char *extension) const {
  if (!initialized) {
    return false;
  }
...
bool SdCard::selectRandomImageFromDirectories(const std::vector<String> &directories, String &out_directory, String &out_file_path,
  ...
  for (const auto &dir : shuffled) {
    ...
    String file_path = this->getFileAtIndexCached(dir.c_str(), out_selected_index, extension, true);
