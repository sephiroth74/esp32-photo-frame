...
SdCardTocParser::SdCardTocParser(SdCard &sdCard, const char *tocDataPath, const char *tocMetaPath)
    : sdCard_(sdCard), tocDataPath_(tocDataPath), tocMetaPath_(tocMetaPath), metaParsed_(false), cachedDirModTime_(0),
      cachedFileCount_(0), cachedFileSize_(0) {}
...
bool SdCardTocParser::validate_toc(const char *expectedPath, const char *expectedExt) {
  ...
  if (expectedPath && cachedDirectoryPath_ != String(expectedPath)) {
    log_v("TOC invalid: directory path mismatch (stored: %s, expected: %s)", cachedDirectoryPath_.c_str(), expectedPath);
    return false;
  }

  // Check extension matches
  if (expectedExt && cachedExtension_ != String(expectedExt)) {
    log_v("TOC invalid: extension mismatch (stored: %s, expected: %s)", cachedExtension_.c_str(), expectedExt);
    return false;
  }
  ...
  if (cachedFileSize_ > 0) {
    ...
    if (actualSize != cachedFileSize_) {
      log_v("TOC invalid: file size mismatch (stored: %u, actual: %u)", cachedFileSize_, actualSize);
      return false;
    }
  }
