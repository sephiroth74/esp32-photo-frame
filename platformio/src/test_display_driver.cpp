...
void setup() {
  ...
  displayDriver = new DisplayDriver6C(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN);
#else
  log_i("Creating B&W display driver...");
  displayDriver = new DisplayDriverBW(EPD_CS_PIN, EPD_DC_PIN, EPD_RST_PIN, EPD_BUSY_PIN, EPD_SCK_PIN, EPD_MOSI_PIN);
#endif
  ...
  for (size_t i = 0; i < IMAGE_BUFFER_SIZE; i++) {
    ...
    if ((x / 100) % 2 == 0) {
      ...
      switch ((x / 100) % 6) {
      case 0:
        test_image_buffer[i] = DISPLAY_COLOR_RED;
        break;
      case 1:
        test_image_buffer[i] = DISPLAY_COLOR_GREEN;
        break;
      case 2:
        test_image_buffer[i] = DISPLAY_COLOR_BLUE;
        break;
      case 3:
        test_image_buffer[i] = DISPLAY_COLOR_YELLOW;
        break;
      default:
        test_image_buffer[i] = DISPLAY_COLOR_WHITE;
        break;
