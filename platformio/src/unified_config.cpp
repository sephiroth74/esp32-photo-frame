...
photo_frame_error_t load_unified_config(SdCard &sdCard, const char *config_path, unified_config &config) {
  ...
  // Supports both array format (v0.11.0+) and legacy object format (v0.10.0 and
  // earlier)
  //
  // New format (v0.11.0+):
  //   "wifi": [
    ...
    if (doc["wifi"].is<JsonArray>()) {
      ...
      for (JsonObject wifiObj : wifiArray) {
        if (config.wifi.network_count >= WIFI_MAX_NETWORKS) {
          log_w("Maximum number of WiFi networks (%d) exceeded, ignoring "
                "additional "
                "entries",
                WIFI_MAX_NETWORKS);
          break;
      ...
      // This ensures existing config.json files continue to work without manual
      // updates
      log_i("Detected legacy WiFi configuration format (single object) - "
            "migrating to array "
            "format");
      ...
      if (ssid.length() > 0 && password.length() > 0) {
        ...
        log_i("Consider updating config.json to new array format for future "
              "compatibility");
      } else {
        log_e("Invalid legacy WiFi configuration (empty SSID or password)");
      }
    } else {
      log_w("WiFi configuration has unsupported format (must be array or "
            "object)");
    }
  } else {
  ...
  if (doc.containsKey("board_config")) {
    ...
    // Load display rotation (0-3) (with backward compatibility for
    // portrait_mode)
  ...
  if (doc.containsKey("google_drive_config")) {
    ...
    config.GoogleDrive.enabled = gd_obj["enabled"] | true; // Default to true for backward compatibility
    ...
    if (gd_obj.containsKey("authentication")) {
      ...
      config.GoogleDrive.auth.service_account_email = auth_obj["service_account_email"].as<String>();
      ...
      log_d("Google Drive authentication - email: %s", config.GoogleDrive.auth.service_account_email.c_str());
      log_d("Google Drive authentication - client ID: %s", config.GoogleDrive.auth.client_id.c_str());
    ...
    if (gd_obj.containsKey("drive")) {
      ...
      config.GoogleDrive.drive.list_page_size =
          min(drive_obj["list_page_size"] | GOOGLE_DRIVE_MAX_LIST_PAGE_SIZE, GOOGLE_DRIVE_MAX_LIST_PAGE_SIZE);
      ...
      if (drive_obj.containsKey("folder_ids")) {
        ...
        log_d("Google Drive configuration - total folders: %d", config.GoogleDrive.drive.folder_ids.size());
      } else {
        ...
        if (legacy_folder_id.length() > 0) {
          ...
          log_w("Google Drive: using legacy folder_id, please migrate to "
                "folder_ids array");
          log_d("Google Drive configuration - folder ID: %s", legacy_folder_id.c_str());
        }
      }
      log_d("Google Drive configuration - root CA path: %s", config.GoogleDrive.drive.root_ca_path.c_str());
      log_d("Google Drive configuration - list page size: %d", config.GoogleDrive.drive.list_page_size);
      log_d("Google Drive configuration - use insecure TLS: %s", config.GoogleDrive.drive.use_insecure_tls ? "true" : "false");
    ...
    if (gd_obj.containsKey("caching")) {
      ...
      config.GoogleDrive.caching.toc_max_age_seconds = cache_obj["toc_max_age_seconds"] | GOOGLE_DRIVE_TOC_MAX_AGE_SECONDS;
    } else {
    ...
    if (gd_obj.containsKey("rate_limiting")) {
      ...
      config.GoogleDrive.rate_limiting.min_request_delay_ms = rate_obj["min_request_delay_ms"] | GOOGLE_DRIVE_MIN_REQUEST_DELAY_MS;
      config.GoogleDrive.rate_limiting.max_retry_attempts = rate_obj["max_retry_attempts"] | GOOGLE_DRIVE_MAX_RETRY_ATTEMPTS;
      config.GoogleDrive.rate_limiting.backoff_base_delay_ms =
          rate_obj["backoff_base_delay_ms"] | GOOGLE_DRIVE_BACKOFF_BASE_DELAY_MS;
      config.GoogleDrive.rate_limiting.max_wait_time_ms = rate_obj["max_wait_time_ms"] | GOOGLE_DRIVE_MAX_WAIT_TIME_MS;
    } else {
...
photo_frame_error_t load_unified_config_with_fallback(SdCard &sdCard, const char *config_path, unified_config &config) {
  ...
  if (result != error_type::None) {
    ...
    // The caller can check config.is_valid() to see if essential config is
    // missing
    return error_type::None;
  }
  ...
  if (!config.GoogleDrive.enabled && !config.sd_card.enabled) {
    log_e("Configuration error: No image source enabled! At least one of "
          "Google Drive or SD "
