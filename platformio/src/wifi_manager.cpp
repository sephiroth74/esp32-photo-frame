...
photo_frame_error_t WifiManager::connect() {
  ...
  for (uint8_t network_idx = 0; network_idx < _network_count && !connected; network_idx++) {
    ...
    for (uint8_t attempt = 0; attempt < maxRetriesPerNetwork && !connected; attempt++) {
      log_i("Connection attempt %u/%u for network: %s", attempt + 1, maxRetriesPerNetwork, ssid.c_str());

      WiFi.begin(ssid.c_str(), password.c_str());
      WiFi.setTxPower(WIFI_POWER_8_5dBm);

      // CRITICAL FIX: Wait for WiFi hardware to initialize before checking
      // status WiFi.begin() is asynchronous and needs time to start the
      // connection process
      delay(1000);
      ...
      if (connection_status == WL_CONNECTED) {
        ...
        log_w("Failed to connect to '%s' (attempt %u/%u)", ssid.c_str(), attempt + 1, maxRetriesPerNetwork);

        // PROBLEM #3 FIX: Use disconnect(false) for faster retries
        // EXCEPTION: ESP32-C6 needs disconnect(true) due to I2C/WiFi
        // interference
        ...
        // Exponential backoff with jitter for retries (only within same
        // network)
