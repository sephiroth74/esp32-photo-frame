...
void WSServer::handleWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
    ...
    case WStype_PING: log_i("[WSServer] Received PING from client %u", num); break;

    case WStype_PONG: log_i("[WSServer] Received PONG from client %u", num); break;
    ...
    default: log_w("[WSServer] Unknown event type: %d", type); break;
    }
}
...
void WSServer::handleControlMessage(uint8_t num, const String& message) {
    ...
    if (String(type) == "init") {
        ...
        photo_frame_error validationResult =
            photo_frame::binary_utils::validatePFR1FileStructure(tmp, true);
        tmp.close();
...
void WSServer::checkUploadTimeout() {
    ...
    if (elapsed > WS_UPLOAD_TIMEOUT_MS) {
        log_w("[WSServer] Upload timeout after %lu ms (limit: %lu ms)",
              elapsed,
              (unsigned long)WS_UPLOAD_TIMEOUT_MS);
