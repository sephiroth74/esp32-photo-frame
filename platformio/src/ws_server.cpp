...
WSServer::WSServer(uint16_t port, WSEventCallback callback)
    : m_port(port), m_callback(callback), m_taskHandle(nullptr), m_running(false), m_webSocket(nullptr), m_connectedClientsCount(0),
      m_activeClientId(255), m_lastPingMs(0), m_lastPongMs(0), m_waitingPong(false), m_uploadActive(false), m_uploadClientId(255),
      m_uploadFilename(""), m_uploadToken(""), m_uploadTimestamp(0), m_uploadOrientation(0), m_uploadBytesReceived(0),
      m_lastUploadActivityMs(0), m_lastActivityMs(0), m_uploadFile(), m_imageBuffer(nullptr), m_imageBufferSize(0),
      m_imageReceivedSize(0), m_receivingImage(false) {
