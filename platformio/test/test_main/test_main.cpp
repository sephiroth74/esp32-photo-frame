...
void test_error_full_constructor() {
  ...
  photo_frame_error error(message, 456, ERROR_SEVERITY_WARNING,
...
void test_error_is_critical() {
  photo_frame_error error("Critical message", 789, ERROR_SEVERITY_CRITICAL,
                          ERROR_CATEGORY_BATTERY);
  TEST_ASSERT_TRUE(error.is_critical());
}

void test_error_is_not_critical() {
  photo_frame_error error("Warning message", 101, ERROR_SEVERITY_WARNING,
                          ERROR_CATEGORY_NETWORK);
...
void test_battery_step_constructor() {
  BatteryMappingStep step(50, 3700);
...
void test_battery_info_constructors() {
  ...
  BatteryInfo info2 = BatteryInfo::full();
  TEST_ASSERT_EQUAL(100, info2.percent);

  BatteryInfo info3 = BatteryInfo::empty();
