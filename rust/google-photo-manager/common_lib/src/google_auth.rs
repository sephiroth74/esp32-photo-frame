...
pub const GOOGLE_REQUIRED_SCOPES: [&str; 5] = [
...
pub fn validate_google_credentials_file(
    ...
    let parsed: GoogleCredentialsFile =
        serde_json::from_str(&raw).map_err(|e| format!("Invalid credentials.json format: {e}"))?;
    ...
    if credentials.auth_provider_x509_cert_url.trim().is_empty() {
        return Err("credentials.json: 'auth_provider_x509_cert_url' is missing or empty".into());
    }
...
pub fn get_valid_access_token(
    ...
    if is_token_expired(token_path).unwrap_or(true) {
        info!("Token is expired or missing, attempting to refresh...");
        match perform_google_oauth_login(token_path, credentials_path) {
            Ok(new_token) => {
                info!("Token refreshed successfully");
                return get_access_token(&new_token);
            }
            Err(e) => {
    ...
    get_access_token(token_path)
}

fn is_token_expired(token_path: &Path) -> Result<bool, Box<dyn std::error::Error>> {
    match get_token_expiration(token_path) {
        Ok(expiration_time) => {
...
fn get_access_token(token_path: &Path) -> Result<String, Box<dyn std::error::Error>> {
    let token_data = read_token(token_path)?;
    Ok(token_data.token.access_token)
}

fn read_token(token_path: &Path) -> Result<GoogleTokenData, Box<dyn std::error::Error>> {
    if !token_path.exists() {
...
fn get_token_expiration(token_path: &Path) -> Result<NaiveDateTime, Box<dyn std::error::Error>> {
    let token_data = read_token(token_path)?;
    ...
    let day_of_year = expires_at[1] as u32;
    let hour = expires_at[2] as u32;
    let min = expires_at[3] as u32;
    let sec = expires_at[4] as u32;
    let nano = expires_at[5] as u32;

    let date = NaiveDate::from_yo_opt(year, day_of_year).ok_or("Invalid day of year")?;
    let time = NaiveTime::from_hms_nano_opt(hour, min, sec, nano).ok_or("Invalid time")?;
