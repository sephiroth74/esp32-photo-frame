...
- **ESP32 Binary Format**: Generates optimized .pfr1 files for direct ESP32 loading
...
./bt-client upload -f processed/image.pfr1 -d PhotoFrame-ABC123

# Upload to first available device (auto-detect)
./bt-client upload -f processed/image.pfr1

# Specify device by MAC address
./bt-client upload -f processed/image.pfr1 -d AA:BB:CC:DD:EE:FF

# Set display orientation during upload (0=landscape, 1=portrait, 2=landscape-reverse, 3=portrait-reverse)
./bt-client upload -f processed/image.pfr1 -d PhotoFrame-ABC123 --orientation 1

# Verbose output with detailed information
./bt-client upload -f processed/image.pfr1 -v
```

Notes:
- The client automatically reads device info (width, height, MTU) and mirrors those values; no size argument is needed.
- Ensure the .pfr1 you upload matches the device’s resolution (e.g., 800x480) and display type (bw or 6c) generated by the processor.
- Options summary (bt-client upload):
  - `-f, --file <FILE>`: required, binary file (.pfr1)
...
./bt-client upload -f ~/processed/bin/output_001.pfr1 --orientation 0
```
...
│   ├── bw_dGVzdF9pbWFnZQ==.pfr1           # Black & white single image
│   ├── 6c_dGVzdF9pbWFnZQ==.pfr1           # 6-color single image
│   ├── combined_bw_ABC123_DEF456.pfr1     # Black & white combined portrait
│   ├── combined_6c_ABC123_DEF456.pfr1     # 6-color combined portrait
│   └── ...
...
└── bin/                    # Only binary files
    ├── bw_dGVzdF9pbWFnZQ==.pfr1           # Black & white single image
    ├── bw_aW1hZ2UyMDI0.pfr1               # Another black & white image
    ├── combined_bw_ABC123_DEF456.pfr1     # Black & white combined portrait
    └── ...
```
...
- **Single images**: `{prefix}_{filename}_{hash}.{ext}`
- **Combined portraits**: `combined_{prefix}_{file1}_{file2}_{hash}.{ext}` (e.g., `combined_bw_ABC123_DEF456.pfr1`)
...
8. **Multi-Format Output**: Generates multiple output formats with organized directory structure:
   ...
   - **Binary**: ESP32-compatible .pfr1 files with 8-bit RRRGGGBB format
