...
/// Uses ONNX-based YOLO11 when the `ai` feature is enabled, otherwise a stub.
...
mod onnx {
    ...
    pub struct SubjectDetectionResult {
        ...
        /// Individual detections for debug visualization
    ...
    impl SubjectDetectionResult {
        ...
        pub fn get_raw_detection_data(&self) -> Result<python_yolo_integration::FindSubjectResult> {
            ...
            let detections: Vec<python_yolo_integration::Detection> = self
                .individual_detections
                .iter()
                .map(|(x_min, y_min, x_max, y_max, confidence)| {
                    python_yolo_integration::Detection {
            ...
            Ok(python_yolo_integration::FindSubjectResult {
                image: "onnx_detection".to_string(),
                imagesize: python_yolo_integration::ImageSize {
        ...
        /// Get per-person detection boxes for debug visualization
        pub fn detection_boxes(&self) -> Vec<(u32, u32, u32, u32, f32)> {
            if !self.individual_detections.is_empty() {
                return self.individual_detections.clone();
            }

            if let Some((x_min, y_min, x_max, y_max)) = self.bounding_box {
                return vec![(x_min, y_min, x_max, y_max, self.confidence)];
            }
...
#[cfg(feature = "ai")]
pub use onnx::{
    SubjectDetectionResult, SubjectDetector, create_default_detector as create_detector,
};
...
mod stub {
    ...
    /// Detection result for smart cropping
    #[derive(Debug, Clone)]
    pub struct SubjectDetectionResult {
        pub center: (u32, u32),
        pub offset_from_center: (i32, i32),
    ...
    pub fn create_detector(verbose: bool) -> Result<SubjectDetector> {
        SubjectDetector::new(verbose)
    }
}

#[cfg(not(feature = "ai"))]
pub use stub::{SubjectDetectionResult, SubjectDetector, create_detector};
